function [Accuracies, N_Trials] = Plot_Accuracies_(Trial_Data, design, Debug_Or_Whole_Screen)
...
for trial = 1:NTrials
    ...
    Trial_RDS_Location_xy_in_Pixels(trial, :) = [condition.x_fixation, condition.y_fixation];
...
RDS_Fixation_Location=design.Fixation_center_xy_in_Pixel;
    ...
    for j = 1:N_FrameDuration
        for k = 1:N_Location
            Selected_Trials  = find(Trial_RDS_Prob_DiskDotIsSameAcrossEyes == Prob_DiskDotIsSameAcrossEyes(i) & ...
                ...
                Trial_RDS_Location_xy_in_Pixels(:, 2) == RDS_Fixation_Location(1,k, 2) );
    ...
    for i = 1:N_Coherence
        for j = 1:N_FrameDuration
            if sum(squeeze(N_Trials(i, j, :)))>0
                ...
                txt2= ['For ',  txt, ' trials, accuracies at ', location_txt, ' are ', num2str(Accuracies(i, j, :))];
                disp(txt2);
            end
        end
    end
