function Give_OpeningMessage(design, SetUp_Info)
    ...
    Opening_Message(1).txt = 'Thank you for participating in this experiment';
    Opening_Message(1).xy = [0.3, 0.3];
    Opening_Message(2).txt = 'we start with a depth screening, to check that you can see stereo depth';
    Opening_Message(2).xy = [0.3, 0.4];
    count =2;

    % if design.Give_Instructions_Or_Not ==1
    %     count = count+1;
    %     Opening_Message(count).txt = 'Then we give you instructions on how to do the experimental trials in this session';
    ...
    count = count+1;
    Opening_Message(count).txt = 'Enter RightArrow Key to proceed';
    Opening_Message(count).xy = [0.3, 0.5];
